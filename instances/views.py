...
from webvirtmgr_v1.settings import QEMU_KEYMAPS, QEMU_CONSOLE_TYPES
from log.views import addlogmsg
...
def instances(request):
    ...
    if request.user.role in ['CU', 'GA']:
        ...
        #TODO:目前只完成了比较有局限性的查找争取尽快完成较为清晰的查找
...
def instances_admin(request, compute_id):
    ...
    #comp = Compute.objects.get(id=compute_id)
    comp = get_object_or_404(Compute, pk=compute_id)
    def get_userinstances_info(instance):
        info = {}
        uis = UserInstance.objects.filter(instance=instance)
        info['count'] = len(uis)
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            addlogmsg(request.user.username, instance.name, lib_err.message)
    pprint(all_user_vms)
    return render(request, 'instances_admin.html', locals())
...
def instance(request, compute_id, vname):
    ...
    :return:
    """
    header_title, path1, path2 = u'虚拟机详情', u'虚拟机管理', u'虚拟机详情'
    error_messages = []
    messages = []
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if 'delete' in request.POST and (request.user.role == 'SU' or userinstace.is_delete):
            ...
            if 'resize' in request.POST and (request.user.role == 'SU' or userinstace.is_change):
                ...
                if not request.user.role == 'SU' and quota_msg:
            ...
            if request.user.role == 'SU' or userinstace.is_vnc:
            ...
            if request.user.role == 'SU' or request.user.userattributes.can_clone_instances:
    ...
    return my_render('instances/instance.html', locals(),request)
